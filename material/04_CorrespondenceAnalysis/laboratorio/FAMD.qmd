...
numeric_cols = pydf.select_dtypes(include=[np.number]).columns

# Obtener las coordenadas de las variables de FAMD
coords = famdPy.column_coordinates_

# Filtrar las coordenadas para solo las variables numéricas usando 'isin()'
numeric_coords = coords[coords.index.isin(numeric_cols)]

# Graficar las variables numéricas con flechas (en negro)
plt.figure(figsize=(8, 8))
plt.axhline(0, color='gray', lw=1)
plt.axvline(0, color='gray', lw=1)

# Graficar las flechas para cada variable numérica
for i in range(len(numeric_coords)):
    plt.arrow(0, 0, numeric_coords.iloc[i, 0], numeric_coords.iloc[i, 1], 
              color='black', alpha=0.7, head_width=0.02, head_length=0.02)
    plt.text(numeric_coords.iloc[i, 0] * 1.1, numeric_coords.iloc[i, 1] * 1.1, 
             numeric_coords.index[i], color='black', ha='center', va='center', fontsize=9)

# Dibujar el círculo unitario (radio = 1)
circle = plt.Circle((0, 0), 1, color='gray', fill=False, linestyle='--', lw=1)
plt.gca().add_artist(circle)

# Ajustar los límites de los ejes para asegurar que el círculo se vea
plt.xlim(-1.5, 1.5)
plt.ylim(-1.5, 1.5)

# Ajustes de la visualización
plt.title('Variables cuantitativas en el plano factorial (FAMD)')
plt.xlabel('Componente 1')
plt.ylabel('Componente 2')
plt.grid(True)
plt.axis('equal')  # Esto asegura que los ejes tengan la misma escala
plt.tight_layout()
...
```{python}
#| label: circulo Unitario Contribuciones-py
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
# Coordenadas de variables (correlación con los componentes)
coords = famdPy.column_correlations(pydf)

# Supongamos que queremos calcular la contribución en Dim 1 y Dim 2
# Contribución aproximada: proporcional al cuadrado de la coordenada
contrib = famdPy.column_contributions_[0] + famdPy.column_contributions_[1]

# Define tus colores del gradiente
gradient_colors = ["#00AFBB", "#E7B800", "#FC4E07"]
custom_cmap = LinearSegmentedColormap.from_list("custom", gradient_colors)

plt.figure(figsize=(8, 8))
sns.set(style="white")

# Normaliza contribuciones para el colormap
norm = plt.Normalize(contrib.min(), contrib.max())
texts = []

for var in coords.index:
    x, y = coords.loc[var, ['Dim 1', 'Dim 2']]
    contrib = coords.loc[var, 'contrib']
    color = custom_cmap(norm(contrib))
    
    plt.arrow(0, 0, x, y, color=color, alpha=0.9, head_width=0.02)
    texts.append(plt.text(x, y, var, color=color))
...
plt.xlabel("Dim 1")
plt.ylabel("Dim 2")
plt.title("Variables cuantitativas (coloreadas por contribución)")
plt.grid(True)
plt.xlim(-1, 1)
plt.ylim(-1, 1)
...
cbar.set_label('Contribución (aproximada)')
...
```{python}
#| label: circulo unitario calidad-py
#| echo: true
#| eval: true
#| warning: false
#| message: false
