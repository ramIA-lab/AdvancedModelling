...
Entre los problemas de diseño más comunes se incluyen:

-   la ubicación

-   el tamaño

-   la legibilidad deficientes del texto

-   selección descuidada de colores ([guía de estilos](https://files.taylorandfrancis.com/TJOM-suppmaterial-quick-guide.pdf))
...
![](https://ichef.bbci.co.uk/ace/ws/640/cpsprodpb/6655/production/_113279162_gettyimages-463899545.jpg.webp){fig-align="center" width="75%"}
:::

::: {.column width="40%"}
![](https://cdn.britannica.com/00/161400-050-E49598A0/John-Snow.jpg){fig-align="right" width="75%"}
:::
:::::
...
El ***modelo de datos raster***, divide la superfície en celdas de tamaño constante en una malla (una matriz) donde cada celda (o *píxel*) tiene un tamaño similar y un valor específico.

Los **rasters** agregan características espacialmente específicas a una resolución dada, lo que significa que son consistentes en el espacio y escalables (muchos conjuntos disponibles en todo el mundo).
...
El **modelo de datos vectoriales** representa el mundo mediante *puntos*, *líneas* y *polígonos* **georreferenciados.** Estos tienen bordes discretos y bien definidos, lo que significa que los conjuntos de datos vectoriales suelen tener un alto nivel de precisión (aunque no necesariamente exactitud).

Se basa en puntos ubicados dentro de un sistema de referencia de coordenadas (CRS).

Los puntos pueden representar características independientes o pueden vincularse para formar geometrías más complejas, como líneas y polígonos.

---

La mayoría de las geometrías de puntos contienen solo dos dimensiones. Tambíen las geometrías tridimensionales (dimension $z$) que normalmente representa la altura sobre el nivel del mar.

:::: columns
:::

![](https://r.geocompx.org/images/vector_projected.png)

|           |                             |
|:---------:|-----------------------------| 
| **GDAL** | Leer, escribir y manipular datos geográficos | 
| **PROJ** | Permite transformar el sistema de coordenadas | 
| **GEOS** | motor de geometría planar para operaciones como el cálculo de zonas de influencia y centroides en datos con CRS proyectadas | 
| **S2** | motor de geometría esférica desarrollado por Google | 

---

## Características simples

Las **características simples** son un modelo de datos jerárquico que representa una amplia gama de tipos de geometría.

Está compuesto por 18 tipos de geometría de las cuales las mas utilizadas son las siguiente 7:

![](https://r.geocompx.org/images/sf-classes.png){fig-align="center"}

Podemos encontrar 3 paquetes muy utilizados:

|           |                                                  |
|:---------:|--------------------------------------------------| 
| **sp**    | clases de datos                                  | 
| **rgdal** | lectura y escritura de datos mediante GDAL y PROJ| 
| **rgeos** | operaciones espaciales mediante GEOS             |
...
Las **características simples** son un modelo de datos ampliamente compatible que sustenta las estructuras de datos en numerosas aplicaciones SIG, como QGIS y PostGIS.

Una ventaja importante es que su uso garantiza la transferencia de su trabajo a otras configuraciones, por ejemplo, al importar y exportar a bases de datos espaciales.
...
Las geometrías son los componentes básicos de las características simples. Podemos encontrar las siguientes geometrías:

-   **Puntos**: La base de cada tipo de geometría es el punto. Un punto es simplemente una coordenada en un espacio bidimensional, tridimensional o cuatridimensional.

    -   `POINT(5 2)`

-   **Cadena de líneas**: Secuencia de puntos con una línea recta que conecta los puntos.

    -   `LINESTRING (1 5, 4 4, 4 1, 2 2, 3 2)`

-   **Polígono**: Secuencia de puntos que forman un anillo cerrado que no se interseca. Se denomina cerrado si el primer y último punto tienen las mismas coordenadas.

    -   `POLYGON ((1 5, 2 2, 4 1, 4 4, 1 5))`

![](https://r.geocompx.org/figures/sfcs-1.png){fig-align="center"}

------------------------------------------------------------------------

El estándar de entidades simple también permite la existencia de múltiples geometrías de un mismo tipo dentro de una misma entidad, dentro de la versión *"multi"* de cada tipo de geometría.

-   **Multipunto**: `MULTIPOINT (5 2, 1 3, 3 4, 3 2)`

-   **Cadena multilineal**: `MULTILINESTRING ((1 5, 4 4, 4 1, 2 2, 3 2), (1 2, 2 4))`

-   **Multipolígono**: `MULTIPOLYGON (((1 5, 2 2, 4 1, 4 4, 1 5), (0 2, 1 2, 1 3, 0 3, 0 2)))`

![](https://r.geocompx.org/figures/multis-1.png){fig-align="center"}

------------------------------------------------------------------------

Finalmente, una colección de geometría puede contener cualquier combinación de geometrías, incluidos (multi)puntos y cadenas de líneas.

-   **Colección de geometría**: `GEOMETRYCOLLECTION (MULTIPOINT (5 2, 1 3, 3 4, 3 2), LINESTRING (1 5, 4 4, 4 1, 2 2, 3 2))`

![](https://r.geocompx.org/figures/geomcollection-1.png){fig-align="center"}
...
El modelo de datos ráster suele constar de un encabezado ráster y una matriz (con filas y columnas) que representa celdas equidistantes (también llamadas píxeles.

El encabezado ráster define:

-   El **CRS**\
-   El **origen** (o punto de partida): Coordenada de la esquina inferior izquierda de la matriz.
-   La **extensión**: Número de columnas, el número de filas y la resolución del tamaño de celda.

La resolución se puede calcular de la siguiente manera:

$$
resolución = \frac{xmax-xmin}{ncol}, \frac{ymax - ymin}{nrow}
$$

Partiendo del origen, podemos acceder y modificar fácilmente cada celda, ya sea usando su ID o especificando explícitamente las filas y columnas.

Esta representación matricial evita almacenar explícitamente las coordenadas de los cuatro vértices de hecho, solo almacena una coordenada, el origen. Esto hace que el procesamiento ráster sea mucho más eficiente y rápido que el procesamiento de datos vectoriales.

A diferencia de los datos vectoriales, la celda de una capa ráster solo puede contener un único valor. El valor puede ser continuo o categórico.

![](https://r.geocompx.org/figures/raster-intro-plot-1.png){fig-align="center"}

Los mapas ráster suelen representar fenómenos continuos como la elevación, la temperatura, la densidad de población o datos espectrales o características discretas, como el suelo o las clases de cobertura terrestre.

![](https://r.geocompx.org/figures/raster-intro-plot2-1.png){fig-align="center"}
...
Los **CRS** permiten identificar con exactitud la posición de los datos sobre el globo terráqueo. Los **CRS** definen como se relacionan los elementos espaciales de los datos con la superfície terrestre.

Los CRS identifican cualquier ubicación en la superficie terrestre mediante dos valores:

-   **Longitud**: Ubicación en dirección este-oeste en distancia angular desde el plano del meridiano de Greenwich.
-   **Latitud**: Distancia angular al norte o al sur del plano ecuatorial.

Las distancias en los CRS geográficos no se miden en metros.

La superficie de la Tierra en los CRS geográficos se representa mediante:

-   **Una superficie esférica**: Asumen que la Tierra es una esfera perfecta de un radio determinado; presentan la ventaja de la simplicidad, pero, al mismo tiempo, son imprecisos, ya que la Tierra no es exactamente una esfera.

-   **Una superficie elipsoidal**: Son ligeramente más precisos y se definen por dos parámetros: el radio ecuatorial (aproximadamente 11.5 km mas largo que el radio polar) y el radio polar. Estos son adecuados porque la Tierra está comprimida. Esta son una parte del *datum*.

------------------------------------------------------------------------

El **datum** contiene información sobre qué elipsoide usar y la relación precisa entre las coordenadas y la ubicación en la superficie de la Tierra. Hay dos tipos de datum:

-   **geocéntrico** (como WGS84)
-   **local** (como NAD83).

![](https://r.geocompx.org/images/02_datum_fig.png){fig-align="center"}
...
### Reproyecciones de datos geográficos
